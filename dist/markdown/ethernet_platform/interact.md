...
3. mode模式功能，支持：action/time, 模式功能需要和结果返回一起使用，默认值为action模式。
  ...
  b. time模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作并记录触发的时间。格式为: ["time", {时间单位}, {等待值(单位:秒)}, {循环次数(选填)}]。当达到用户给定的等待值，系统会返回每次触发的时间间隔(时间间隔的单位由用户给定，目前支持秒("s")/毫秒("ms")/微秒("us"))。当数据达到缓存上限且尚未达到等待时间时，系统会自动返回缓存。当循环次数为1时或未指定时，系统会在不断循环;当循环次数是正整数时，系统会以等待值执行给定的循环次数。
