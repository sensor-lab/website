...
3. mode模式功能，支持：action/time, 模式功能需要和结果返回一起使用，默认值为action模式。
  a. action模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作。如果定义了结果返回数组，会将硬件操作的结果返回给指定目的地。
  b. time模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作并记录触发的时间。格式为: ["time", {时间单位}, {等待值(单位:秒)}, {循环次数(选填)}]。当达到用户给定的等待值，系统会返回每次触发的时间间隔(时间间隔的单位由用户给定，目前支持秒("s")/毫秒("ms")/微秒("us"))。当数据达到缓存上限且尚未达到等待时间时，系统会自动返回缓存。当循环次数为0时或未指定时，系统会在不断循环;当循环次数是正整数时，系统会以等待值执行给定的循环次数。
...
例如，统计管脚26脉冲中高电平的触发间隔(返回值单位毫秒，假定脉冲初始为低电平)，等待10秒，只进行一次循环。结束后将结果返回给192.168.1.1的端口8000:
{
  "event": "pinstate",
  "pin": 26,
  "mode":["time","ms", 10, 1],
...
假设上面的例子的返回值为:[12,16,21,15,10,13]。则在10秒钟的时间里，一共发生了7次上升沿触发，第一次和第二次上升沿的时间间隔为12毫秒，第二次和第三次上升沿的时间间隔为16毫秒，以此类推。且在此次结束后，不会再进行循环。
