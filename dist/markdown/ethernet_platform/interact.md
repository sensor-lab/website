...
3. mode模式功能，支持：action/time/count, 模式功能需要和结果返回一起使用，默认值为action模式。
  a. action模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作。如果定义了结果返回数组，会将硬件操作的结果返回给指定目的地。
  b. time模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作。但结果返回只会在满足触发条件的第二次以及以后才会返回给指定目的地，且返回当次和上一次触发的时间间隔。time模式需要提供时间单位参数，支持"s"(秒)或者"ms"(毫秒)。
  c. count模式：当触发条件满足时，系统会执行所有在actions数组中定义的硬件操作。但结果返回只会在指定时间间隔之后才会返回给指定目的地，且返回时间间隔之内该触发条件发生的次数。count模式需要提供时间单位参数和时间间隔参数，支持"s"(秒)或者"ms"(毫秒)。
...
例如，在统计管脚26脉冲中高电平的长度(单位毫秒，假定脉冲初始为低电平)，并将结果返回给192.168.1.1的端口8000:
{
  "event": "pinstate",
  "pin": 26,
  "mode":["time","ms"],
...
例如，统计管脚26在5秒种里，发生了多少次脉冲，并将结果返回给192.168.1.1的端口8000:
{
  "event": "pinstate",
  "pin": 26,
  "mode":["count","s",5],
